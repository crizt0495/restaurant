...
export function RolesClient({ roles, permissions, rolePermissions }: RolesClientProps) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="font-display text-2xl font-bold tracking-tight md:text-3xl leading-none">Peran & Izin</h2>
        <p className="mt-1.5 text-sm font-medium text-muted-foreground">Kelola hak akses per role</p>
      </div>
                  ...
                  <CardContent className="pt-3">
                    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                      {perms.map((pm) => (
                        <label
                          key={pm.key}
                          className="flex cursor-pointer items-center gap-2 rounded-xl border border-border bg-card px-3 py-2 text-sm shadow-sm transition-all duration-150 hover:border-primary/30 hover:shadow-md hover:-translate-y-0.5"
                        >
                          <Checkbox
