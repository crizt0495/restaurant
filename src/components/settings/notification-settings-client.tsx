...
export function NotificationSettingsClient({ initial }: { initial?: Partial<NotificationPrefs> }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-2xl font-bold tracking-tight md:text-3xl leading-none">Pengaturan Notifikasi</h2>
        <p className="mt-1.5 text-sm font-medium text-muted-foreground">Atur notifikasi apa yang ingin diterima</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 pt-4 sm:grid-cols-2">
          {ITEMS.map((item) => (
            <div key={item.key} className="flex items-center justify-between rounded-xl border border-border bg-card p-3 shadow-sm">
              <div>
